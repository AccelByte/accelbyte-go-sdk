...
// Deprecated: 2022-01-10 - please use ImagesStorageShort instead.
func (aaa *ImagesService) ImagesStorage(input *images.ImagesStorageParams) (*amsclientmodels.APIImageStorage, error) {
	token, err := aaa.TokenRepository.GetToken()
	if err != nil {
		return nil, err
	}
	ok, unauthorized, forbidden, notFound, internalServerError, err := aaa.Client.Images.ImagesStorage(input, client.BearerToken(*token.AccessToken))
	if unauthorized != nil {
		return nil, unauthorized
	}
	if forbidden != nil {
