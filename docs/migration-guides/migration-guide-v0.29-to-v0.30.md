...
```
/*
`ConvertInterfaceToModel` utils is reducing its required parameter.
You can make your own utils to convert the model or using the existing one. 

Before: `ConvertInterfaceToModel` has 2 parameters and using global variable. This is not the best practice.
After: `ConvertInterfaceToModel` is only need one parameter.

Here is the differences of the implementation
*/
```
