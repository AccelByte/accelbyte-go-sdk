...
func (aaa *GameSessionService) GetSessionServerSecret(input *game_session.GetSessionServerSecretParams) (*sessionclientmodels.ApimodelsServerSecret, error) {
	...
	ok, badRequest, unauthorized, forbidden, notFound, internalServerError, err := aaa.Client.GameSession.GetSessionServerSecret(input, client.BearerToken(*token.AccessToken))
	if badRequest != nil {
		return nil, badRequest
	}
	if unauthorized != nil {
