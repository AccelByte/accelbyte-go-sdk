...
// Deprecated: 2022-01-10 - please use AdminDeletePlatformCredentialsByPlatformIDShort instead.
func (aaa *PlatformCredentialService) AdminDeletePlatformCredentialsByPlatformID(input *platform_credential.AdminDeletePlatformCredentialsByPlatformIDParams) error {
	token, err := aaa.TokenRepository.GetToken()
	if err != nil {
		return err
	}
	_, badRequest, unauthorized, forbidden, notFound, internalServerError, err := aaa.Client.PlatformCredential.AdminDeletePlatformCredentialsByPlatformID(input, client.BearerToken(*token.AccessToken))
	if badRequest != nil {
		return badRequest
	}
	if unauthorized != nil {
...
// Deprecated: 2022-01-10 - please use AdminSyncPlatformCredentialsShort instead.
func (aaa *PlatformCredentialService) AdminSyncPlatformCredentials(input *platform_credential.AdminSyncPlatformCredentialsParams) (*sessionclientmodels.ApimodelsXblCertificateResponseBody, error) {
	token, err := aaa.TokenRepository.GetToken()
	if err != nil {
		return nil, err
	}
	ok, badRequest, unauthorized, forbidden, notFound, internalServerError, err := aaa.Client.PlatformCredential.AdminSyncPlatformCredentials(input, client.BearerToken(*token.AccessToken))
	if badRequest != nil {
		return nil, badRequest
	}
	if unauthorized != nil {
