...
lint:
	...
	find -type f -iname go.mod -exec dirname {} \; | while read DIRECTORY; do \
		echo "$$DIRECTORY"; \
			docker run -t --rm -u $$(id -u):$$(id -g) -v $$(pwd):/data/ -w /data/ -e GOCACHE=/data/.cache/go-build -e GOLANGCI_LINT_CACHE=/data/.cache golangci/golangci-lint:v1.42.1\
...
lint-mod-outdated:
	...
	find -type f -iname go.mod -exec dirname {} \; | while read DIRECTORY; do \
		echo "$$DIRECTORY"; \
		docker run -t --rm -u $$(id -u):$$(id -g) -v $$(pwd):/data/ -w /data/ -e GOCACHE=/data/.cache/go-build golang:1.16-alpine3.15 \
...
samples:
	find -type f -name main.go -exec dirname {} \; | xargs -I{} \
			sh -c 'echo "{}" && docker run -t --rm -v $$(pwd):/data/ -w /data/ -e GOCACHE=/data/.cache/go-build golang:1.16-alpine3.15 \
...
test_integration:
	...
	docker run -t --rm --env-file $(ENV_FILE_PATH) -v $$(pwd):/data/ -w /data/ -e GOCACHE=/data/.cache/go-build golang:1.16-alpine3.15 \
			sh -c "CGO_ENABLED=0 GOOS=linux go test github.com/AccelByte/accelbyte-go-sdk/services-api/pkg/tests/integration"

test_cli:
	@test -n "$(SDK_MOCK_SERVER_PATH)" || (echo "SDK_MOCK_SERVER_PATH is not set" ; exit 1)
	rm -f test.err
	docker run -t --rm -v $$(pwd):/data/ -w /data/ -e GOCACHE=/data/.cache/go-build golang:1.16-alpine3.15 \
