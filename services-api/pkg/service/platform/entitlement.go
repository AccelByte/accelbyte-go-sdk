...
// Deprecated: 2022-01-10 - please use QueryEntitlementsByItemIdsShort instead.
func (aaa *EntitlementService) QueryEntitlementsByItemIds(input *entitlement.QueryEntitlementsByItemIdsParams) (*platformclientmodels.EntitlementPagingSlicedResult, error) {
	token, err := aaa.TokenRepository.GetToken()
	if err != nil {
		return nil, err
	}
	ok, err := aaa.Client.Entitlement.QueryEntitlementsByItemIds(input, client.BearerToken(*token.AccessToken))
	if err != nil {
		return nil, err
	}
...
func (aaa *EntitlementService) QueryEntitlementsByItemIdsShort(input *entitlement.QueryEntitlementsByItemIdsParams) (*platformclientmodels.EntitlementPagingSlicedResult, error) {
	authInfoWriter := input.AuthInfoWriter
	if authInfoWriter == nil {
		security := [][]string{
			{"bearer"},
