...
func Handler(evt *events.LambdaFunctionURLRequest) (*events.LambdaFunctionURLResponse, error) {
	...
	getStatCode, errStatCode := statConfigService.GetStatShort(inputStatCode)
	if errStatCode != nil {
	...
	return &events.LambdaFunctionURLResponse{
		StatusCode: 200,
		Body:       fmt.Sprintf("Successfully get stat user item %v and stat config %v", getUserStatItem.Data, *getStatCode.StatCode),
	}, nil
}
