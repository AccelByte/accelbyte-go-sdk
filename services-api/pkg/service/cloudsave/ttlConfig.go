...
// Deprecated: 2022-01-10 - please use DeleteAdminGameRecordTTLConfigShort instead.
func (aaa *TTLConfigService) DeleteAdminGameRecordTTLConfig(input *ttl_config.DeleteAdminGameRecordTTLConfigParams) error {
	token, err := aaa.TokenRepository.GetToken()
	if err != nil {
		return err
	}
	_, badRequest, unauthorized, forbidden, notFound, internalServerError, err := aaa.Client.TTLConfig.DeleteAdminGameRecordTTLConfig(input, client.BearerToken(*token.AccessToken))
	if badRequest != nil {
		return badRequest
	}
	if unauthorized != nil {
